...
data "azurerm_application_insights" "msk-prod-application-insight-data" {
...
data "azurerm_storage_account" "msk-prod-storage-account" {
...
data "azurerm_user_assigned_identity" "msk-prod-managed-identity" {
...
resource "azurerm_resource_group" "az-msk-prod-add-employee-rg" {
...
resource "azurerm_storage_account" "az-msk-addemployee-storage-account" {
  ...
  resource_group_name      = azurerm_resource_group.az-msk-prod-add-employee-rg.name
  location                 = azurerm_resource_group.az-msk-prod-add-employee-rg.location
...
resource "azurerm_function_app" "az-msk-prod-function-app" {
  ...
  app_settings = {
    ...
    APPINSIGHTS_INSTRUMENTATIONKEY = data.azurerm_application_insights.msk-prod-application-insight-data.instrumentation_key
    FUNCTIONS_WORKER_RUNTIME       = "dotnet"
    DEV_STORAGE_ACCOUNT            = "mskdevstorage428"
    STAGE_STORAGE_ACCOUNT          = "mskstagestorage428"
    PROD_STORAGE_ACCOUNT           = "mskprodstorage428"
    EMPLOYEES_TABLE                = "mskEmployees"
    ...
    PROD_MANAGED_IDENTITY_URL      = data.azurerm_user_assigned_identity.msk-prod-managed-identity.client_id
  }
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [data.azurerm_user_assigned_identity.msk-prod-managed-identity.id]
  }
