...
resource "azurerm_resource_group" "msk-stage-storage-account-resource-group" {
...
resource "azurerm_storage_account" "msk-stage-storage-account" {
  name                      = var.msk-storage-account-name
  resource_group_name       = azurerm_resource_group.msk-stage-storage-account-resource-group.name
  location                  = azurerm_resource_group.msk-stage-storage-account-resource-group.location
...
resource "azurerm_storage_container" "msk-stage-storage-account-container" {
  for_each              = toset(var.msk-list-of-storage-account-container-names)
  name                  = each.key
  storage_account_name  = azurerm_storage_account.msk-stage-storage-account.name
  container_access_type = "blob"
  depends_on = [
    azurerm_storage_account.msk-stage-storage-account
  ]
}

resource "azurerm_storage_table" "msk-stage-storage-account-table" {
...
resource "azurerm_role_assignment" "assign-storage-table-contributor-role-to-user-managed-identity" {
  scope                = azurerm_storage_account.msk-stage-storage-account.id
